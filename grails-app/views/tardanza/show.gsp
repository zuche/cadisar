
<%@ page import="cadisar.Tardanza" %>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
        <meta name="layout" content="main" />
        <g:set var="entityName" value="${message(code: 'tardanza.label', default: 'Tardanza')}" />
        <title><g:message code="default.show.label" args="[entityName]" /></title>
    </head>
    <body>
        <div class="nav">
            <span class="menuButton"><a class="home" href="${createLink(uri: '/')}"><g:message code="default.home.label"/></a></span>
            <span class="menuButton"><g:link class="list" action="list"><g:message code="default.list.label" args="[entityName]" /></g:link></span>
            <span class="menuButton"><g:link class="create" action="create"><g:message code="default.new.label" args="[entityName]" /></g:link></span>
        </div>
        <div class="body">
            <h1><g:message code="default.show.label" args="[entityName]" /></h1>
            <g:if test="${flash.message}">
            <div class="message">${flash.message}</div>
            </g:if>
            <div class="dialog">
                <table>
                    <tbody>
                    
                        <tr class="prop">
                            <td valign="top" class="name"><g:message code="tardanza.id.label" default="Id" /></td>
                            
                            <td valign="top" class="value">${fieldValue(bean: tardanzaInstance, field: "id")}</td>
                            
                        </tr>
                    
                        <tr class="prop">
                            <td valign="top" class="name"><g:message code="tardanza.description.label" default="Description" /></td>
                            
                            <td valign="top" class="value">${fieldValue(bean: tardanzaInstance, field: "description")}</td>
                            
                        </tr>
                    
                        <tr class="prop">
                            <td valign="top" class="name"><g:message code="tardanza.dia.label" default="Dia" /></td>
                            
                            <td valign="top" class="value">${fieldValue(bean: tardanzaInstance, field: "dia")}</td>
                            
                        </tr>
                    
                        <tr class="prop">
                            <td valign="top" class="name"><g:message code="tardanza.mes.label" default="Mes" /></td>
                            
                            <td valign="top" class="value">${fieldValue(bean: tardanzaInstance, field: "mes")}</td>
                            
                        </tr>
                    
                        <tr class="prop">
                            <td valign="top" class="name"><g:message code="tardanza.año.label" default="Año" /></td>
                            
                            <td valign="top" class="value">${fieldValue(bean: tardanzaInstance, field: "año")}</td>
                            
                        </tr>
                    
                        <tr class="prop">
                            <td valign="top" class="name"><g:message code="tardanza.justified.label" default="Justified" /></td>
                            
                            <td valign="top" class="value"><g:formatBoolean boolean="${tardanzaInstance?.justified}" /></td>
                            
                        </tr>
                    
                        <tr class="prop">
                            <td valign="top" class="name"><g:message code="tardanza.student.label" default="Student" /></td>
                            
                            <td valign="top" class="value"><g:link controller="alumno" action="show" id="${tardanzaInstance?.student?.id}">${tardanzaInstance?.student?.encodeAsHTML()}</g:link></td>
                            
                        </tr>
                    
                    </tbody>
                </table>
            </div>
            <div class="buttons">
                <g:form>
                    <g:hiddenField name="id" value="${tardanzaInstance?.id}" />
                    <span class="button"><g:actionSubmit class="edit" action="edit" value="${message(code: 'default.button.edit.label', default: 'Edit')}" /></span>
                    <span class="button"><g:actionSubmit class="delete" action="delete" value="${message(code: 'default.button.delete.label', default: 'Delete')}" onclick="return confirm('${message(code: 'default.button.delete.confirm.message', default: 'Are you sure?')}');" /></span>
                </g:form>
            </div>
        </div>
    </body>
</html>
